...
<div :style='{"border":"1px solid #dfdfdf","padding":"20px","margin":"20px auto 0","borderRadius":"16px","background":"#fff","width":"1200px","position":"relative"}'>
    <el-button :style='{"border":"0","cursor":"pointer","padding":"0 10px","margin":"0 5px 0 0","outline":"none","color":"#fff","borderRadius":"4px","background":"rgba(255, 107, 157, 1)","width":"auto","lineHeight":"40px","fontSize":"14px","height":"40px"}' type="warning" size="mini" @click="backClick" class="el-icon-back">返回</el-button>
    <div class="section-title" :style='{"margin":"0px 0","color":"#ffffff","borderRadius":"8px 8px 0 0","textAlign":"center","background-color":"#FF6B9D","fontSize":"24px","lineHeight":"150px","fontWeight":"bold"}'>我的订单</div>
                        ...
                        if(item.type==1||item.type==3) {
                            ...
                            this.$http.post(table+'/update', this.user).then(res => {
                                ...
                                this.$http.post(`orders/update`, item).then(res => {
                                    if(item.tablename == `dianzichanpin`){
                                        // 加回库存数量
                                        good.alllimittimes = good.alllimittimes + item.buynumber;
                                        this.$http.post(`dianzichanpin/update`, good).then(res => {
                                        });
                                    }
                                    this.$message({
                                      message: '退款成功',
